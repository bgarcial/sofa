...
try:\n\
    ...
    // Add the directories listed in the SOFAPYTHON27_PLUGINS_PATH environnement
    // variable (colon-separated) to sys.path
    char * pathVar = getenv("SOFAPYTHON27_PLUGINS_PATH");
    if (pathVar != NULL)
    {
    ...
    PyRun_SimpleString("from SofaPython27.livecoding import onReimpAFile");
}
  ...
  namespace {
    ...
    struct raii {
      raii() {
          ...
          // when an application is directly linking with the SofaPython27 library
